...
9. [published_posts](#9-published_posts)
10. [How Everything Connects](#10-how-everything-connects)
11. [Content Flow: End to End](#11-content-flow-end-to-end)
...
| `social_accounts` | jsonb | YES | — | JSON object storing the social media handles/URLs for this avatar (e.g., {"tiktok": "url"}). |
| `tiktok_follower_history` | jsonb | YES | — | JSON array storing daily snapshots of TikTok followers (e.g., [{"date": "2026-06-25", "followers": 1500}]). |
| `instagram_follower_history` | jsonb | YES | — | JSON array storing daily snapshots of Instagram followers. |
| `youtube_follower_history` | jsonb | YES | — | JSON array storing daily snapshots of YouTube followers/subscribers. |
| `facebook_follower_history` | jsonb | YES | — | JSON array storing daily snapshots of Facebook followers. |
| `twitter_follower_history` | jsonb | YES | — | JSON array storing daily snapshots of Twitter/X followers. |
| `linkedin_follower_history` | jsonb | YES | — | JSON array storing daily snapshots of LinkedIn followers. |
...
**Purpose:** Stores the final published state and single-snapshot analytics of posts generated by avatars. This table is completely decoupled from the `content_pipeline`. When a post is published, a row is created here and its metrics are updated continually by a scraper.

| Column | Type | Nullable | Default | Purpose |
|---|---|---|---|---|
| `published_post_id` | uuid | NO | gen_random_uuid() | Primary key. Unique identifier for the published post. |
| `owner_avatar_id` | uuid | NO | — | FK to `avatar_details`. The avatar representing this post. |
| `owner_user_id` | uuid | NO | — | FK to `users`. The user who owns this post. |
| `platform` | text | NO | — | The platform where the post is published (e.g. tiktok, instagram). |
| `post_url` | text | NO | — | The live URL to the published post. |
| `caption` | text | YES | — | The text caption attached to the post. |
| `posted_at` | date | YES | — | The date the post was published. |
| `impressions` | bigint | YES | 0 | Total times the post was shown (impression count). |
| `likes` | bigint | YES | 0 | Current like count. |
| `comments` | bigint | YES | 0 | Current comment count. |
| `shares` | bigint | YES | 0 | Current share count. |
| `saves` | bigint | YES | 0 | Current save/bookmark count. |
| `reposts` | bigint | YES | 0 | Current repost count. |
| `engagement_rate` | numeric | YES | — | Calculated rate of engagement (e.g. engagements / views). |
| `virality` | numeric | YES | — | The raw virality calculation metric. |
| `engagement_score` | numeric | YES | — | Standardized score reflecting overall engagement health. |
| `virality_score` | numeric | YES | — | Standardized score reflecting algorithmic push. |
| `created_at` | timestamptz | NO | now() | When the record was created in the database. |

**Notes:**
- This table uses a **single snapshot** approach for analytics. The columns are updated in place with the latest scraped metrics rather than maintaining complex historical JSON arrays.
- Because it's completely detached from the pipeline, it acts strictly as an analytics engine for the avatar's outward-facing social presence.
